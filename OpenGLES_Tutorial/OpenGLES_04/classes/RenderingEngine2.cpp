...
void RenderingEngine2::BindTexture(GLuint textureId) const
{
    GLuint textureSlot = glGetUniformLocation(m_simpleProgram, "inputImageTexture");
...
void RenderingEngine2::Render() const
{
    ...
    GLuint textureCoordinateSlot = glGetAttribLocation(m_simpleProgram, "InputTextureCoordinate");
    ...
    const GLvoid* pTextureCoords = &Vertices[0].TexturePosition[0];
    
    glVertexAttribPointer(positionSlot, 3, GL_FLOAT, GL_FALSE, stride, pCoords);
    glVertexAttribPointer(textureCoordinateSlot, 2, GL_FLOAT, GL_FALSE, stride, pTextureCoords);
    
    GLsizei vertexCount = sizeof(Vertices) / sizeof(Vertex);
    glDrawArrays(GL_TRIANGLES, 0, vertexCount);
    
    glDisableVertexAttribArray(positionSlot);
    glDisableVertexAttribArray(textureCoordinateSlot);
}
